...
export default function Landing(){
    ...
    const handelGenre = (e) =>{
        ...
        if(!filterData.genre.includes(val)){
            ...
            setFilterData({...filterData,genre:filterData.genre.filter(g =>{
                return g !== val;
            })});
        }
    ...
    const postVideoUpload = async(formData)=>{
        ...
        const {videoLink,imageLink,title,genre,contentRating,releaseDate } = formData;
        let d= new Date(releaseDate);
        let dateString =`${d.getDate()} ${monthNames[d.getMonth()]} ${d.getFullYear()}`;
        let data ={
            "videoLink": videoLink,
            "title": title,
            "genre": genre,
        ...
        try{
            ...
            getVideosByFilteredData(filterData);
        }catch(e){
            console.log(e)
            if (e.response) {
                enqueueSnackbar(e.response.data.message, { variant: "error" });
    ...
    const validateData= (formData)=>{
        const {videoLink,imageLink,title,genre,contentRating,releaseDate } = formData;
        if(videoLink === "" || imageLink==="" || title === "" || genre === "" || contentRating ==="" || releaseDate === ""){
    ...
    useEffect(()=>{
        getVideosBySort(videosList,sortBy);
    },[sortBy])
    useEffect(()=>{
        getVideosByFilteredData(filterData);
    },[filterData])
    return(
        <>
            <Header upload={<DialogButton postVideoUpload={postVideoUpload} 
                                         open={open} 
                                         setOpen={setOpen} 
            ...
            >
                <TextField 
                        sx={{backgroundColor:"#D4D2D2",borderRadius: "10px",color:"#1818185c "}}
            ...
            ></TextField>
            <GenerePanel 
                handleSort={handleSort} sortBy={sortBy} filterData={filterData}
